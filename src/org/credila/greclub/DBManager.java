...
public class DBManager extends Service {
    ...
    public List<String> read_random(String box)
	{
        ...
        int max = 335;
        Random r = new Random();
        int rand_ID = r.nextInt(max - min + 1) + min;
        
        Cursor cursor=null;
    	Log.d("read_random: ", String.valueOf(rand_ID));
    	if(box.equals("HF"))
    	{	Log.d("HF","inside HF");
        	query = "SELECT distinct word, gremeaning FROM wordindex WHERE wid >"+rand_ID+" and wid <"+(rand_ID+6);
    	}
    	else
    	{
        	query = "SELECT distinct word, gremeaning FROM wordindex WHERE word LIKE '"+box+"%' and wid >"+rand_ID+" and wid <"+(rand_ID+6);   		
    	}
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                meaning=cursor.getString(cursor.getColumnIndexOrThrow("gremeaning"));
                // Adding word to list
