...
public class FinalPhase extends Activity {
	int numQuestions=0;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setTitle("Final Phase");
//		getActionBar().setDisplayHomeAsUpEnabled(true);
		ListView ls=(ListView)findViewById(R.id.questionlist);
		try
		{
			InputStream fileStream=getResources().openRawResource(getResources().getIdentifier("raw/fpquestions", "raw", getPackageName()));
			...
			ls.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View arg1,
					...
					InputStream fileStream=getResources().openRawResource(getResources().getIdentifier("raw/fpa"+arg2, "raw", getPackageName()));
					int len=fileStream.available();
					byte fileBuffer[]=new byte[len];
					fileStream.read(fileBuffer);
					fileStream.close();
					...
					catch(Exception e){Log.v("Final Phase OnClick","Exception Occured OnItemClick: ");e.printStackTrace(System.out);}
				}
			});
		}
		catch(Exception e)
		{
			Log.v("Final Phase","Exception Occured: "+e);
		}
	}
